...
init([Type, ConnName, Node, Bucket, ExtModule, InitArgs]) ->
    ...
    dcp_traffic_spy:node_alive(Node, [{bucket, Bucket}, {type, Type}]),
    {ok, State#state{
           last_update = now(),
           ext_state = ExtState
          }, ?HIBERNATE_TIMEOUT}.
...
get_node_and_type(State) ->
    {Type, _, Node, _} = State#state.connect_info,
    {Node, Type}.
...
handle_packet(<<Magic:8, Opcode:8, _Rest/binary>> = Packet,
    ...
    NewState1 = update_liveness(NewState, Opcode),
    {ok, NewState1#state{ext_state = NewExtState}}.

update_liveness(State, OpCode) ->
    LastUpdate = State#state.last_update,
    case timer:now_diff(now(), LastUpdate) of
        T when T > ?LIVENESS_UPDATE_INTERVAL ->
            {Type, _, Node, Bucket} = State#state.connect_info,
            ?log_debug("~p Updating Liveness for ~p ~p ~n", [dcp_commands:command_2_atom(OpCode), Type, Node]),
            dcp_traffic_spy:node_alive(Node, [{bucket, Bucket}, {type, Type}]),
